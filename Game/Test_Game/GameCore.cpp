...
void GameCore::loadingScreen() {//cuz why not
	graphic->clearBuffer();
	graphic->createFrame(0, 0, 145, 40);
...
void GameCore::clearLevelScreen(int Level, int Score) {
	int top = 15;
	int left = 57;
...
void GameCore::firstLevelScreen() {
	int top = 15;
	int left = 57;
...
int GameCore::playScreen1(int Level, int baseScore)
{
	// Stage clear
...
int GameCore::playScreen(int Level, int baseScore )
{
	mciSendString(song_game_1, NULL, 0, NULL);
	if (!soundOn)
		mciSendString(L"pause song_game_1.wav", NULL, 0, NULL);
...
void GameCore::saveScreen(int score, int mode, int level) {
	int top = 8;
	int left = 58;
...
bool GameCore::pauseScreen(int score, int mode, int level) {
	int top = 15;
	int left = 60;
...
void GameCore::gameoverScreen(int score = 0) {
	int top = 8;
	int left = 46;
	graphic->clearBuffer();
	if (soundOn)
...
bool GameCore::checkCollision(vector<GameLane*> lanes)
{
