...
GameLane::GameLane(int lane, int level, int dir, int green, int red, GraphicsController*& graphic) :
	lane(lane),
	dir(dir),
...
// For level mode
GameLane::GameLane(int lane, int level, int dir, int green, int red, GraphicsController*& graphic, int bg, bool order, int Level) :
	lane(lane),
	dir(dir),
	green(green* FRAMERATE),
	red(red* FRAMERATE)
...
void GameLane::setPassed(bool s) { 
	passed = s; 
}
void GameLane::setNewPassed(bool s) { 
	newPass = s; 
}
bool GameLane::Passed() { 
	return passed; 
}
bool GameLane::NewPass() { 
	return newPass; 
}

coord GameLane::getPos() {
...
void GameLane::render(GraphicsController*& graphic, int offset) {
	// Render lane itself
...
// For level mode
void GameLane::render(GraphicsController*& graphic, int offset, int bg) {
	// Render lane itself
	if (obs.size() == 0)
		return;
	// Render obstacles
...
bool GameLane::checkCollision(BOUNDINGBOX player)
{
	for (auto it : obs)
	{
